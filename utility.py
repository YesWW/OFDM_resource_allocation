...
class Buffer(Dataset):
    def __init__(self, graph, power_alloc, beam_alloc, action, act_log_prob, value, ongoing, target, device):
        ...
        self._power_alloc = power_alloc.cpu()
        self._beam_alloc = beam_alloc.cpu()
        self._action = action.cpu()
        self._act_log_prob = act_log_prob.cpu()
        self._value = value.cpu()
        self._ongoing = ongoing.cpu()
        self._target = target.cpu()

        self._reward = None
        self._return = None
        self._idx = []

        # [step, sample] indexing
        num_step = torch.sum(self._ongoing.int(), dim=0)
        for s in range(self._batch_size):
            n = int(num_step[s].item())
            self._idx.append(np.stack((
                s * np.ones((n,)),
                np.arange(0, n)
            ), axis=1).astype(int))
...
def collate_fn(samp):
    ...
    out['power_alloc'] = torch.cat(out['power_alloc'], dim=0)
    out['beam_alloc'] = torch.cat(out['beam_alloc'], dim=0)
...
def get_buffer_dataloader(buf, batch_size, shuffle=True):
    return DataLoader(buf, batch_size=batch_size, shuffle=shuffle, collate_fn=collate_fn)
...
def collate_fn_step(batch):
    ...
    return {
        'graph': batch_graph,
        'power_alloc': torch.stack([b['power_alloc'] for b in batch], dim=0),
        'beam_alloc': torch.stack([b['beam_alloc'] for b in batch], dim=0),
        'link': torch.tensor([b['link'] for b in batch], dtype=torch.long),
        'rb': torch.tensor([b['rb'] for b in batch], dtype=torch.long),
